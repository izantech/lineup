...
When your spawn prompt indicates Ollama tools are available, delegate text
formatting and prose expansion to a local Ollama model. You make all design
decisions — Ollama only handles mechanical text formatting.

### When to use Ollama

When `OLLAMA_AVAILABLE = true`:
...
2. Call `mcp__ollama__ollama_generate` with your draft and a focused expansion prompt
   (e.g., "Expand these bullet points into clear acceptance criteria prose for a
   developer agent")
